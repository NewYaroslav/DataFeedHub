#pragma once
#ifndef _DTH_DYNAMIC_SPREAD_PROCESSOR_HPP_INCLUDED
#define _DTH_DYNAMIC_SPREAD_PROCESSOR_HPP_INCLUDED

/// \file DynamicSpreadProcessor.hpp
/// \brief Implementation of the dynamic spread restoration processor.

namespace dfh::core {

    /// \class DynamicSpreadProcessor
    /// \brief Processor for restoring bid/ask spreads dynamically based on tick data.
    ///
    /// This processor restores bid/ask prices based on the last trade price and tick flags,
    /// dynamically adjusting the spread value depending on the market conditions and the tick's history.
    class DynamicSpreadProcessor final : public ISpreadProcessor {
    public:

        virtual ~DynamicSpreadProcessor() = default;

        /// \brief Processes ticks and restores bid/ask spreads dynamically.
        ///
        /// This method processes the provided tick data, calculates bid/ask prices
        /// dynamically using the differences between consecutive ticks, segments the data
        /// into one-second intervals, and updates the buffer's state with the latest tick.
        ///
        ...
        void process(
            ...
            if (has_prev_data) {
                ...
                m_prev_spread = utils::pow10<double>(price_digits) * static_cast<double>(bidask_config.fixed_spread);
            }
            ...
            if (tick.has_flag(TickUpdateFlags::TICK_FROM_BUY)) {
                if (has_prev_data && prev_tick.has_flag(TickUpdateFlags::TICK_FROM_SELL) && tick.last > prev_tick.last) {
                    m_prev_spread = utils::normalize_double(tick.last - prev_tick.last, price_digits);
                }

                tick.ask = tick.last;
                tick.bid = tick.ask - m_prev_spread;
            } else
            if (tick.has_flag(TickUpdateFlags::TICK_FROM_SELL)) {
                if (has_prev_data && prev_tick.has_flag(TickUpdateFlags::TICK_FROM_BUY) && tick.last < prev_tick.last) {
                    m_prev_spread = utils::normalize_double(prev_tick.last - tick.last, price_digits);
                }

                tick.bid = tick.last;
                tick.ask = tick.bid + m_prev_spread;
            } else {
            ...
            for (size_t i = 1; i < ticks.size(); ++i) {
                ...
                MarketTick& ptick = ticks[i - 1];

                if (tick.has_flag(TickUpdateFlags::LAST_UPDATED)) {
                    if (tick.has_flag(TickUpdateFlags::TICK_FROM_BUY)) {
                        if (ptick.has_flag(TickUpdateFlags::TICK_FROM_SELL) && tick.last > ptick.last) {
                            m_prev_spread = utils::normalize_double(tick.last - ptick.last, price_digits);
                        }
                    ...
                    if (tick.has_flag(TickUpdateFlags::TICK_FROM_SELL)) {
                        if (ptick.has_flag(TickUpdateFlags::TICK_FROM_BUY) && tick.last < ptick.last) {
                            m_prev_spread = utils::normalize_double(ptick.last - tick.last, price_digits);
                        }
...
#endif // _DTH_DYNAMIC_SPREAD_PROCESSOR_HPP_INCLUDED
