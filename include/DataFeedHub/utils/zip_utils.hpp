...
namespace dfh::utils {
    ...
    inline void extract_zip_in_memory(
        ...
        // Initialize the stream with provided data.
        mz_stream_mem_set_buffer(mem_stream, const_cast<void*>(static_cast<const void*>(zip_data.data())), zip_data.size());
        if (mz_stream_open(mem_stream, nullptr, MZ_OPEN_MODE_READ) != MZ_OK) {
        ...
        // Close zip archive and memory stream
        mz_zip_close(zip_handle);
        mz_zip_delete(&zip_handle);
        mz_stream_mem_delete(&mem_stream);
    }
    ...
    inline std::string extract_first_file_from_zip(const std::string& zip_data) {
        std::vector<std::pair<std::string, std::string>> output_files;
    ...
    inline std::string extract_from_gzip(const std::string& gzip_data, size_t max_bytes = 2000000000) {
        ...
        decompressor.decompress(decompressed, gzip_data.data(), gzip_data.size());
        return decompressed;
    }
