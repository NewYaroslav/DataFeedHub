...
int run_tick_codec_quote_roundtrip() {
#ifndef _MSC_VER
    std::vector<dfh::QuoteTick> ticks;
    constexpr std::uint64_t base_time = 1700000000000ULL;
    for (std::uint64_t i = 0; i < 10; ++i) {
        dfh::QuoteTick tick{
            101.125 + static_cast<double>(i) * 0.05,
            101.000 + static_cast<double>(i) * 0.05,
            base_time + i * 123ULL};
    ...
    std::vector<dfh::QuoteTick> decoded;
...
int main() {
    return run_tick_codec_quote_roundtrip();
}
