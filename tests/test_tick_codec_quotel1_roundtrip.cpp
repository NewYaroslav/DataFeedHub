...
int run_tick_codec_quotel1_roundtrip() {
#ifndef _MSC_VER
    std::vector<dfh::QuoteTickL1> ticks;
    constexpr std::uint64_t base_time = 1700000000000ULL;
    for (std::uint64_t i = 0; i < 10; ++i) {
        dfh::QuoteTickL1 tick{
            102.125 + static_cast<double>(i) * 0.03,
            101.995 + static_cast<double>(i) * 0.03,
            10.0 + static_cast<double>(i) * 0.5,
            8.0 + static_cast<double>(i) * 0.4,
            base_time + i * 123ULL};
    ...
    std::vector<dfh::QuoteTickL1> decoded;
    ...
    for (std::size_t index = 0; index < ticks.size(); ++index) {
        ...
        assert(almost_equal(expected.ask_volume, actual.ask_volume));
        assert(almost_equal(expected.bid_volume, actual.bid_volume));
    }
...
int main() {
    return run_tick_codec_quotel1_roundtrip();
}
