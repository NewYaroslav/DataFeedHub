...
int run_tick_codec_quotevol_roundtrip() {
#ifndef _MSC_VER
    std::vector<dfh::QuoteTickVol> ticks;
    constexpr std::uint64_t base_time = 1700000000000ULL;
    for (std::uint64_t i = 0; i < 10; ++i) {
        dfh::QuoteTickVol tick{
            101.125 + static_cast<double>(i) * 0.03,
            100.995 + static_cast<double>(i) * 0.03,
            5.0 + static_cast<double>(i) * 0.5,
            base_time + i * 123ULL};
    ...
    std::vector<dfh::QuoteTickVol> decoded;
...
int main() {
    return run_tick_codec_quotevol_roundtrip();
}
